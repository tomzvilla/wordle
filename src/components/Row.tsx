        ...
        <div className={`flex flex-row my-1 ${shouldShake ? 'animate-horizontal-shake' : ''}`}>
            {
                letters.map( (letter, index) => (
                    <div 
                        ...
                        className={`w-[50px] h-[50px] md:w-[70px] md:h-[70px] mx-0.5 p-0 rounded-md flex items-center justify-center border-2 border-gray-300 ${defaultStyles[index]} ${rowCompleted ? `animate-flip` : ''}`} 
                        key={index}
                        onAnimationEnd={() => handleAnimationEnd(index)}
                    >
                        <div className='text-2xl md:text-4xl font-bold dark:text-white'>
                        {letter}
                        </div>
                    </div>
                ))
